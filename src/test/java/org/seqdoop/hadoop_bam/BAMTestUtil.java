...
public class BAMTestUtil {
  ...
  public static File writeCramFile(int numPairs, SAMFileHeader.SortOrder sortOrder, File refFile)
      throws IOException {
    return writeBamFile(numPairs, sortOrder, ".cram", ".crai", refFile);
  }

  public static File writeBamFile(int numPairs, SAMFileHeader.SortOrder sortOrder, String extension, String indexExtension, File refFile)
      throws IOException {
    ...
    if (sortOrder.equals(SAMFileHeader.SortOrder.coordinate)) {
      if (extension.equals(CramIO.CRAM_FILE_EXTENSION)) {
        OutputStream out = new FileOutputStream(new File(bamFile.getAbsolutePath() + indexExtension));
        ...
        BAMIndexer.createIndex(samReader, new File(bamFile.getAbsolutePath()
            .replaceFirst(extension + "$", indexExtension)));
      }
    }
